...
export const login: T_Controller = async (req, res) => {
  ...
  try {
    ...
    return res.status(200).json({ access_token, role: user.role, verified: user.verified });
  } catch (error) {
...
export const register: T_Controller = async (req, res) => {
  ...
  try {
    ...
    const response = await Db.user.register(email, name, password_hash);
    if ('is_error' in response) return custom_error(res, 500, response.message);
